...
void InitializeTextWindow() {
  ...
  DrawTextbox(
    *text_window->InnerWriter(),
    {0, 0},
    text_window->InnerSize()
  );
...
void DrawTextCursor(bool visible) {
  ...
  const auto color = visible
    ? ToColor(0)
    : ToColor(0xffffff);

  const auto pos = Vector2D<int> {
    4 + 8 * text_window_index,
    5
  };
