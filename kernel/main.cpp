...
void MouseObserver(uint8_t buttons,
  ...
  if (!previous_left_pressed && left_pressed) {
    auto layer = layer_manager->FindLayerByPosition(
      mouse_position,
      mouse_layer_id
    );
    if (layer) {
...
void SwitchEhci2Xhci(const pci::Device& xhc_dev) {
  ...
  uint32_t superspeed_ports = pci::ReadConfReg(
    xhc_dev,
    0xdc
  ); // USB3PRM
  ...
  uint32_t ehci2xhci_ports = pci::ReadConfReg(
    xhc_dev,
    0xd4
  ); // XUSB2PRM
