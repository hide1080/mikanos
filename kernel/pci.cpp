...
namespace {
  ...
  /*
   * 指定のファンクションをdeviceに追加する.
   * PCI-PCIブリッジならばセカンダリバスに対してScanBusを実行する.
   */
  Error ScanFunction(uint8_t bus,
                     uint8_t device,
                    uint8_t function) {

    auto class_code = ReadClassCode(
      bus,
      device,
      function
    );

    auto header_type = ReadHeaderType(
      bus,
      device,
      function
    );
    ...
    if (class_code.Match(0x06U, 0x04U)) {
      ...
      auto bus_numbers = ReadBusNumbers(
        bus,
        device,
        function
      );
  ...
  /*
   * 指定のデバイスのファンクションをスキャンする.
   * 有効なファンクションを見つけたらScanFunctionを実行する.
   */
  Error ScanDevice(uint8_t bus,
                   uint8_t device) {
  ...
  /**
   * @brief 指定されたMSIケイパビリティ構造を読み取る.
   * @param dev MSIケイパビリティを読み込むPCIデバイス
   * @param cap_addr MSIケイパビリティレジスタのコンフィグレーション空間アドレス
   */
  MSICapability ReadMSICapability(const Device& dev,
                                  uint8_t cap_addr) {
...
namespace pci {
  ...
  WithError<uint64_t> ReadBar(Device& device,
    ...
    if (bar_index >= 6) {
      return {
        0,
        MAKE_ERROR(Error::kIndexOutOfRange)
      };
    }
...
void InitializePCI() {
  ...
  for (int i = 0; i < pci::num_device; i++) {
    ...
    auto class_code = pci::ReadClassCode(
      dev.bus,
      dev.device,
      dev.function
    );
