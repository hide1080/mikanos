...
namespace {
  ...
  WithError<int> MakeArgVector(char* command,
                                   char* first_arg,
                                   char** argv,
                                   int argv_len,
                                   char* argbuf,
  ...
  void ListAllEntries(FileDescriptor& fd, uint32_t dir_cluster) {
    const auto kEntriesPerCluster = fat::bytes_per_cluster
...
Rectangle<int> Terminal::InputKey(uint8_t modifire,
                                  uint8_t keycode,
                                  char ascii) {
...
void Terminal::Scroll1() {
  ...
  window_->Move(
    ToplevelWindow::kTopLeftMargin + Vector2D<int>{4, 4},
    move_src
  );
  FillRectangle(
    *window_->InnerWriter(),
...
void Terminal::ExecuteLine() {
  ...
  if (strcmp(command, "echo") == 0) {
    ...
    for (int i = 0; i < pci::num_device; i++) {
      ...
      auto vendor_id = pci::ReadVendorId(
        dev.bus,
        dev.device,
        dev.function
      );
...
WithError<int> Terminal::ExecuteFile(fat::DirectoryEntry& file_entry,
  ...
  const uint64_t elf_next_page = (app_load.vaddr_end + 4095)
...
void Terminal::Print(const char* s, std::optional<size_t> len) {
  ...
  Vector2D<int> draw_pos {
    ToplevelWindow::kTopLeftMargin.x,
    cursor_before.y
  };
  ...
  while (true) {
    ...
    switch (msg->type) {
      case Message::kTimerTimeout:
        ...
        if (show_window && window_isactivate) {
...
size_t TerminalFileDescriptor::Read(void* buf,
                                    size_t len) {
...
size_t TerminalFileDescriptor::Write(const void* buf,
                                    size_t len) {
...
size_t TerminalFileDescriptor::Load(void* buf,
                                    size_t len,
                                    size_t offset) {
  return 0;
}
...
size_t PipeDescriptor::Write(const void* buf,
                             size_t len) {
