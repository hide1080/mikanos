...
void Window::DrawTo(FrameBuffer& dst,
                Vector2D<int> pos,
...
void DrawWindow(PixelWriter& writer, const char* title) {
  ...
  auto fill_rect = [&writer](Vector2D<int> pos,
                             Vector2D<int> size,
                             uint32_t c) {
    FillRectangle(
      writer,
  ...
  fill_rect({1, 1},         {win_w -2, 1},          0xc6c6c6);
...
void DrawTextbox(PixelWriter& writer,
  ...
  fill_rect(
    pos + Vector2D<int> {0, size.y},
    {size.x, 1},
    0xc6c6c6
  );
  fill_rect(
    pos + Vector2D<int> {size.x, 0},
    {1, size.y},
    0xc6c6c6
  );
}